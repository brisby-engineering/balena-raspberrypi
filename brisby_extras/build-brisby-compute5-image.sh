...
usage() {
    ...
    echo "  --diagnose             Check why build might produce stock image (template, bblayers, paths); then exit (run on BUILD HOST only, not on device)"
    echo "  -h, --help             Show this help"
    echo ""
    echo "Output: image and artifacts in ${BUILD_SPACE}/"
    echo ""
    echo "On-device panel diagnostic (after flashing custom image): /usr/bin/check-panel-on-device.sh"
    echo "  If missing on device, run from host: balena ssh <device> 'sh -s' < brisby_extras/check-panel-on-device.sh"
    echo ""

 ...
 */
 ...
 struct jadard_panel_desc {
	 ...
	 u32 num_init_cmds;
	 bool use_madctl_param;	/* if true, MADCTL (0x36) value comes from madctl_0x36 module param */
 };
 ...
 /*
  * MADCTL (0x36) value for panels that set use_madctl_param (e.g. txw990002b0).
  * Default 0x26; for 90° rotation try 0x06, 0x26, 0x66, 0xA6, 0xC6.
  */
 static int madctl_0x36 = 0x26;
 module_param(madctl_0x36, int, 0644);
 MODULE_PARM_DESC(madctl_0x36, "Value written to MADCTL register 0x36 (txw990002b0 only). Default 0x26; for 90° rotation try 0x06, 0x26, 0x66, 0xA6, 0xC6.");
 ...
 static int jadard_enable(struct drm_panel *panel)
 {
	 ...
	 unsigned int i;
	 int err;
	 u8 cmd_buf[JD9365DA_INIT_CMD_LEN];
	 ...
	 for (i = 0; i < desc->num_init_cmds; i++) {
		 ...
		 cmd_buf[0] = cmd->data[0];
		 if (cmd->data[0] == 0x36 && desc->use_madctl_param)
			 cmd_buf[1] = (u8)madctl_0x36;
		 else
			 cmd_buf[1] = cmd->data[1];
 
		 err = mipi_dsi_dcs_write_buffer(dsi, cmd_buf, JD9365DA_INIT_CMD_LEN);
		 if (err < 0)
			 return err;
	 }
 ...
 static const struct jadard_init_cmd txw990002b0_init_cmds[] = {
   { .data = { 0xE0, 0x00 } },
   ...
   { .data = { 0xE0, 0x01 } },// SET_PAGE=1: analog/power/VCOM/gamma + panel behavior tuning
   ...
   { .data = { 0x34, 0x0A } },
   { .data = { 0x35, 0x04 } },
   /* MADCTL 0x36: overridden by module param madctl_0x36 when use_madctl_param (default 0x26; 90° try 0x06, 0x26, 0x66, 0xA6, 0xC6) */
   { .data = { 0x36, 0x26 } },
   { .data = { 0x37, 0x1F } },
   { .data = { 0x38, 0x1F } },
   { .data = { 0x39, 0x1F } },
 ...
 static const struct jadard_panel_desc txw990002b0_desc = {
   .mode = {
	 .clock        = 78340,
	 ...
	 .hsync_start  = 720 + 30,
	 .hsync_end    = 720 + 30 + 20,
	 .htotal       = 720 + 30 + 20 + 30,// total = 800
 
	 .vdisplay     = 1600,
	 .vsync_start  = 1600 + 16,
	 .vsync_end    = 1600 + 16 + 2,
	 .vtotal       = 1600 + 16 + 2 + 14, //total = 1632
   ...
   .lanes = 4,
   .format = MIPI_DSI_FMT_RGB888,
   .init_cmds = txw990002b0_init_cmds,
   .num_init_cmds = ARRAY_SIZE(txw990002b0_init_cmds),
   .use_madctl_param = true,
 };
 ...
 static const struct of_device_id jadard_of_match[] = {
   ...
   {
	 .compatible = "cw,txw990002b0",
	 .data = &txw990002b0_desc
   },
	 { /* sentinel */ }
 };
 ...
 MODULE_AUTHOR("Jamie Nichol <jamie@brisby.com>");
